...
# input files can be found at the Google Drive location shared by Nhan

df1 = pd.read_csv('../transaction_data/Transactions-0.csv')

df2 = pd.read_csv('../transaction_data/Transactions-1.csv')

df3 = pd.read_csv('../transaction_data/Transactions-2.csv')

end = time.time()

print("The total time taken was {} seconds".format(end-start))

# concatenating dataframes into one dataset
start = time.time()
...
data_og.drop(data_og[data_og.pk == 'pk'].index[0], inplace=True)

data_og = data_og.reindex()

# removing previous datasets from memory
del df1,df2,df3
...
# creating function to query dictionary
def search_tz(country_name):
    for  country, tz in country_timezone_dict.items():
        if country == country_name:
            return tz

# updating time format to be UTC for first_event_time + timezone_awareness = location_local_time       
local_time_updated = []
local_missing_country = []
errors = []

for index, row in data_localtz[['origin_country','first_event_time']].iterrows():
    try:
        local_tz_code = str(search_tz(row[0]))
        localtimezone = pytz.timezone(local_tz_code)
        local_time = localtimezone.localize(row[1])
        utc_time = local_time.astimezone(pytz.utc)
        local_time_updated.append(utc_time)
    except Exception as e:
        errors.append(index)
        local_time_updated.append('NA')
        local_missing_country.append(row[0])

# adding /utc updated first event time to data
data_localtz.insert(loc=len(data_localtz.columns),column = 'first_event_time_utc',value=local_time_updated)
...
output = []
for x in local_missing_country:
    if x not in output:
        output.append(x)
print ("These are the coutries missing from the dictionary above " + str(output))
...
data.to_csv('total_six_month_rescrape_data_labels_order_10days.csv')


# The cells below must be inlcluded in the encoding steps as they are the transformations required before the data be sent to the AWS Endpoint
...
# create a df with all non-numerical columns as a reference 
non_numerical_data = data[['pk','parcel_id','org_id','org_slug','carrier_id','parcel_product','imported_date',
                          'first_event_time', 'timedelta_in_seconds','first_event_time_utc','timedelta_in_secs',
                          'timedelta_in_mins','timedelta_in_hrs','origin_counts','timezone_awareness',
                          'org_carrier_count', 'origin_counts','carrier_count']]

#Dropping 'org_carrier_count' and 'org_carrier'
data.drop(['pk','parcel_id','org_id','org_slug','carrier_id','parcel_product','imported_date',
                          'first_event_time', 'timedelta_in_seconds','first_event_time_utc','timedelta_in_secs',
                          'timedelta_in_mins','timedelta_in_hrs','origin_counts','timezone_awareness',
                          'org_carrier_count', 'origin_counts','timedelta_in_mins','carrier_count'], axis=1, inplace = True)


# In[9]:


# adjusting label column
...
# creating a copy of the data to join back after prediction
...
# one hot encoding the NN
# creating dummied data for model 

for_dummy = data.drop('Label',axis=1)

for_dummy = for_dummy.replace(np.nan, 0, regex=True)

for_dummy = for_dummy.astype('object')

dummy = pd.get_dummies(for_dummy,drop_first=True)

label_data = data.Label

data_dummy = data.merge(dummy,left_index=True,right_index=True)

removing_cols = data.drop('Label',axis=1).columns

data = data_dummy.drop(labels=removing_cols,axis=1)
...
# creating X and y variables without column headers
